...
infiniopStatus_t cpuCreateReduceMaxDescriptor(infiniopHandle_t,
                                         ReduceMaxCpuDescriptor_t *desc_ptr,
    ...
    *desc_ptr = new ReduceMaxCpuDescriptor{
        DevCpu,
        data->dt,
	data->shape,
	data->ndim
...
template<typename T>
infiniopStatus_t reduce_max_cpu(ReduceMaxCpuDescriptor_t desc, void *reduced, void const *data, void const *axes) {
...
infiniopStatus_t cpuReduceMax(ReduceMaxCpuDescriptor_t desc,
                        ...
                        void *stream) {
    if (desc->dtype == F16) {
        return reduce_max_cpu<uint16_t>(desc, reduced, data, axes);
    }
    if (desc->dtype == F32) {
        return reduce_max_cpu<float>(desc, reduced, data, axes);
    }
    return STATUS_BAD_TENSOR_DTYPE;
}
