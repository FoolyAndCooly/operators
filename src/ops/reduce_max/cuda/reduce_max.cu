...
template<typename T>
infiniopStatus_t reduce_max_nv_gpu(ReduceMaxCudaDescriptor_t desc, void *reduced, void const *data, void const *axes, void* stream){
...
infiniopStatus_t cudaReduceMax(ReduceMaxCudaDescriptor_t desc,
    ...
    if (desc->dtype == F16) return reduce_max_nv_gpu<half>(desc, reduced, data, axes, stream);
    if (desc->dtype == F32) return reduce_max_nv_gpu<float>(desc, reduced, data, axes, stream);
}
