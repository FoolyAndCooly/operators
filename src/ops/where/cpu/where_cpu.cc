...
infiniopStatus_t cpuCreateWhereDescriptor(infiniopHandle_t,
    ...
    if (!isValidBroadcastShape(a, b, c) || !isValidBroadcastShape(c, condition)) {
        return STATUS_BAD_TENSOR_SHAPE;
    }
    if (!is_contiguous(a) || !is_contiguous(b) || !is_contiguous(c) || !is_contiguous(condition)) {
...
infiniopStatus_t where_cpu(WhereCpuDescriptor_t desc, void const *condition, void *c, void const *a, void const *b) {
	...
	auto index = compactToFlat(indices, desc->condition_strides, desc->ndim);

        if constexpr (std::is_same<Tdata, uint16_t>::value) {
            c_[i] = f32_to_f16(condition_[index] ? f16_to_f32(a_[a_index]) : f16_to_f32(b_[b_index]));
        } else {
            c_[i] = condition_[index] ? a_[a_index] : b_[b_index];
        }
    }
    return STATUS_SUCCESS;
}
...
infiniopStatus_t cpuWhere(WhereCpuDescriptor_t desc,
			void const *condition,
                        ...
                        void *stream) {
    if (desc->dtype == F16) {
        return where_cpu<uint16_t>(desc, condition, c, a, b);
    }
    if (desc->dtype == F32) {
        return where_cpu<float>(desc, condition, c, a, b);
    }
    return STATUS_BAD_TENSOR_DTYPE;
}
